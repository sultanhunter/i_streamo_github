...
class GithubRepoDetailsScreen extends StatefulWidget {
  ...
  const GithubRepoDetailsScreen({Key? key, required this.githubRepo})
      : super(key: key);
...
class _GithubRepoDetailsScreenState extends State<GithubRepoDetailsScreen> {
  ...
  void initState() {
    ...
    context
        .read<GithubRepoDetailCubit>()
        .getRepoData(widget.githubRepo.fullName);
  }
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          title: Row(
            children: [
              Hero(
                tag: widget.githubRepo.name,
                child: CircleAvatar(
                  radius: 16,
                  backgroundImage: CachedNetworkImageProvider(
                    widget.githubRepo.user.getAvatarUrlSmall,
                  ),
              ...
              Text(widget.githubRepo.name)
            ],
          )),
      body: SafeArea(
          child: BlocConsumer<GithubRepoDetailCubit, GithubRepoDetailState>(
        listener: (context, state) {
          if (state is GithubRepoDetailDataReceived) {
            print(state.isFresh);
            if (!state.isFresh) {
              showNoConnectionToast(
                  'No Connection, data may be outdated', context);
            }
          }
        },
        builder: (context, state) {
            ...
            return WebView(
              ...
              initialUrl: Uri.dataFromString(
                '''
                        ...
                        <body>${state.githubRepoDetail.html}</body>
                        </html>
                        $css
                        ''',
                mimeType: 'text/html',
          ...
          } else if (state is GithubRepoDetailError) {
            return Center(
